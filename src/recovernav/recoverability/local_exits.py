...
def local_exit_score(env: GridEnvironment, cell: Cell, radius: int = 3) -> float:
    """Structural recoverability score in [0, 1], not a recovery probability."""
    if radius < 1:
    ...
    for r, c in reachable:
        if r == rmin and rmin < r0: sides.add("N")
        if r == rmax and rmax > r0: sides.add("S")
        if c == cmin and cmin < c0: sides.add("W")
        if c == cmax and cmax > c0: sides.add("E")
    local_free = sum(env.is_free((r, c)) for r in range(rmin, rmax + 1) for c in range(cmin, cmax + 1))
    exit_component = len(sides) / 4.0
    area_component = len(reachable) / max(local_free, 1)
    return float(np.clip(0.75 * exit_component + 0.25 * area_component, 0.0, 1.0))
